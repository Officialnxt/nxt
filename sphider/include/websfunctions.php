...
<?php if ($search_results['did_you_mean']){?>
	...
	<?php echo $sph_messages['DidYouMean'];?>: <a href="<?php print 'results.php?query='.quote_replace(addmarks($search_results['did_you_mean'])).'&search=1'?>"><?php print $search_results['did_you_mean_b']; ?></a>?
...
<!-- results listing -->
	...
	<?php foreach ($qry_results as $_key => $_row){
		...
		<?php print utf8_decode($fulltxt); ?><hr />
        
		<?php  if ($domain_name==$last_domain && $merge_site_results == 1 && $domain == "") {?>
		<a href="<?php print 'results.php?query='.quote_replace(addmarks($query)).'&amp;search=1&amp;results='.$results_per_page.'&amp;domain='.$domain_name?>">More results from <?php print $domain_name?></a><br />
...
?>
	...
	<?php if ($start >1){?>
				...
				<a href="<?php print 'results.php?query='.quote_replace(addmarks($query)).'&amp;start='.$prev.'&amp;search=1&amp;results='.$results_per_page.$type_qry.$adv_qry.'&amp;domain='.$domain?>"><?php print $sph_messages['Previous']?></a>
	<?php  }?>	

	<?php  foreach ($other_pages as $page_num) {
				if ($page_num !=$start){?>
					<a href="<?php print 'results.php?query='.quote_replace(addmarks($query)).'&amp;start='.$page_num.'&amp;search=1&amp;results='.$results_per_page.$type_qry.$adv_qry.'&amp;domain='.$domain?>"><?php print $page_num?></a>
				<?php } else {?>	
	...
	<?php if ($next <= $pages){?>	
			<a href="<?php print 'results.php?query='.quote_replace(addmarks($query)).'&amp;start='.$next.'&amp;search=1&amp;results='.$results_per_page.$type_qry.$adv_qry.'&amp;domain='.$domain?>"><?php print $sph_messages['Next']?></a>
	<?php  }?>
